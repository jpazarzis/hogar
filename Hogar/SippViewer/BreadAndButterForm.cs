...
namespace SippViewer
{
    public partial class BreadAndButterForm : Form
    {
        ...
        private readonly SippOddsRetriever _oddsRetriever;
        ...
        void UpdateGrid()
        {
            ...
            if (null == _race || _race.Count(h => h.IsBestBet) != 1)
            {
        ...
        void UpdateSurfAndTurf()
        {
            ...
            SippHorse bestBest = _race.Find(h => h.IsBestBet);

            var alreadyUsed = new List<int>();


            _race.Where(h => h.IsContender && false == h.IsBestBet && h.Scratched == false && h.ProgramNumberWithoutEntryChar != bestBest.ProgramNumberWithoutEntryChar).ToList().ForEach
                (
                    h =>
                    {
                        if (false == alreadyUsed.Contains(h.ProgramNumberWithoutEntryChar))
                        {
                            alreadyUsed.Add(h.ProgramNumberWithoutEntryChar);
                            int rowIndex = _gridSurfAndTurf.Rows.Add();
                            _gridSurfAndTurf.Rows[rowIndex].Cells[0].Value = string.Format("{0} X {1} (Exacta)", bestBest.ProgramNumberWithoutEntryChar, h.ProgramNumberWithoutEntryChar);
            ...
            _tbAmountNeededForSurfAndTurf.Text = string.Format(cultureInfo, "{0:0}", totalAmountNeeded);
            _tbSurfAndTurfRate.Text = string.Format(cultureInfo,"{0:0.0} - 1 ", (((double)_cbAmountToWin.SelectedItem) / totalAmountNeeded) - 1);
        ...
        void UpdateUsingOnlyExactasGrid()
        {
                ...
                (
                    h =>
                    {
                        if (false == alreadyUsed.Contains(h.ProgramNumberWithoutEntryChar))
                        {
                            ...
                            _gridExactasOnly.Rows[rowIndex].Cells[0].Value = string.Format("{0} X {1} (Exacta)", bestBest.ProgramNumberWithoutEntryChar, h.ProgramNumberWithoutEntryChar);
                ...
                (
                    h =>
                    {
                        if (false == alreadyUsed.Contains(h.ProgramNumberWithoutEntryChar))
                        {
                            alreadyUsed.Add(h.ProgramNumberWithoutEntryChar);
                            int rowIndex = _gridExactasOnly.Rows.Add();
                            _gridExactasOnly.Rows[rowIndex].Cells[0].Value = string.Format("{0} X {1} (Exacta)", h.ProgramNumberWithoutEntryChar, bestBest.ProgramNumberWithoutEntryChar);
            ...
            _amountNeededForExactaOnly.Text = string.Format(cultureInfo, "{0:0}", TotalAmountNeededForExactaOnly);
            _rateForExactaOnly.Text = string.Format(cultureInfo, "{0:0.0} - 1 ", (((double)_cbAmountToWin.SelectedItem) / TotalAmountNeededForExactaOnly) - 1);
        ...
        void UpdateUsingOnlyWinGrid()
        {
            ...
            _gridUsingOnlyWin.Rows[rowIndex].Cells[0].Value = string.Format("{0} (TO WIN)", bestBest.ProgramNumberWithoutEntryChar.ToString());
                ...
                (
                    h =>
                    {
                        if (false == alreadyUsed.Contains(h.ProgramNumberWithoutEntryChar))
                        {
                            ...
                            _gridUsingOnlyWin.Rows[rowIndex].Cells[0].Value = string.Format("{0} X {1} (Exacta)", h.ProgramNumberWithoutEntryChar, bestBest.ProgramNumberWithoutEntryChar);
            ...
            _tbAmountNeeded.Text = string.Format(cultureInfo,"{0:0}", TotalAmountNeeded);
            _tbRate.Text = string.Format(cultureInfo, "{0:0.0} - 1 ", (((double)_cbAmountToWin.SelectedItem) / TotalAmountNeeded) - 1);
