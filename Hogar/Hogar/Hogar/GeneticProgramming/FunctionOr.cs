...
namespace Hogar.GeneticProgramming
{
    sealed class FunctionOr : Node
    {
        ...
        {
            return new FunctionOr(minid, maxid, parent);
        }

        private FunctionOr(int minid, int maxid, Node parent)
        {
            _parent = parent;
        ...
        private FunctionOr(FunctionOr other, Node parent)
        {
            _parent = parent;
        ...
        {
            return new FunctionOr(this, parent);
        }

        public bool IsEqual(Node other)
        {
            var f = other as FunctionOr;
        ...
        public string GetCodeAsScriptUsingFullNames()
        {
            ...
            return string.Format(" ( {0} ( {1} {2} ) ) ", "OR", _left.GetCodeAsScriptUsingFullNames(), _right.GetCodeAsScriptUsingFullNames());
        }

        public string GetCodeAsScript()
        {
            return string.Format(" ( {0} ( {1} {2} ) ) ", "OR", _left.GetCodeAsScript(), _right.GetCodeAsScript());
        }
